...
public partial class Auth_Deposit_Fee : System.Web.UI.Page
{
    ...
    protected void fvSubmit_Click(object sender, EventArgs e)
    {
        using (TransactionScope ts = new TransactionScope())
        {
            ...
            {
                string chrep = Common.Get(objsql.GetSingleValue("select reciptno from recipt_details where reciptno='" + txtrecipt.Text + "'"));
                if (chrep != null && chrep != "")
                {
                    goto yy;
                }
            ...
            xx:
                if (txtpalert.Text == "" && txtpalert.Text == null)
                {
                    Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Please Fill Pending Alert Date')", true);
                }
        yy:
            if (chrep != null && chrep != "")
            {
                    Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Receipt Already Used')", true);
            }
                ;
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Please Enter Recipt No')", true);
            }
    ...
    protected void txtoldreceiptno_TextChanged(object sender, EventArgs e)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Number is already saved.Chosse a new one.')", true);
        }
    }
    ...
    protected void bill()
    {
        ...
        if (dtbill.Rows.Count > 0)
        {
            ...
            {
                restatus = "False";
                Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Sorry Recipt No Is Finished')", true);
            }
    ...
    private void gettimedetail()
    {
        ...
        {
            Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('" + ss.Message + "')", true);
